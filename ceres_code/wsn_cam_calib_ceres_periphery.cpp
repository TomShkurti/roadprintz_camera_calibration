...
void solve(
	const std::vector<Eigen::Vector2d> & xy_pixels_vec,
	const std::vector<Eigen::Vector2d> & xy_targets_vec,
	const std::vector<Eigen::Vector3d> & xyz_sled_vec,
	std::ofstream & output
){
	    // Build the problem.
    Problem problem;
  ...
  for (int i = 0; i < kNumObservations; ++i) {
	...
	output << std::to_string(rms_err) << ", " << std::to_string(intrinsics[0]) << ", " << std::to_string(intrinsics[1]);
	output << ", " << std::to_string(intrinsics[2]) << ", " << std::to_string(intrinsics[3]);
	output << ", " << std::to_string(intrinsics[4]) << ", " << std::to_string(intrinsics[5]);
	output << ", " << std::to_string(intrinsics[6]) << ", " << std::to_string(intrinsics[7]);
	output << ", " << std::to_string(intrinsics[8]) << ", " << std::to_string(nlines) << "\n";
}
...
int main(int argc, char** argv) {
	...
	std::ofstream output;
	output.open("/home/tes77/ros_ws/src/roadprintz_camera_calibration/intermediate/radial_condense.csv");
	
	int s = xy_pixels_vec.size();
	
	for(int i = 0; i < s; i++){
	
		printf("\n\n%d\n", i);
	
		solve(xy_pixels_vec, xy_targets_vec, xyz_sled_vec, output);
		
		int maxradind = -1;
		double maxrad = -1.0;
		for(int j = 0; j < xy_pixels_vec.size(); j++){
			double rad = sqrt(pow(xy_pixels_vec[j].x(), 2) + pow(xy_pixels_vec[j].y(), 2));
			if(rad > maxrad){
				maxrad = rad;
				maxradind = j;
			}
		}
		xy_pixels_vec.erase(xy_pixels_vec.begin() + maxradind);
		xy_targets_vec.erase(xy_targets_vec.begin() + maxradind);
		xyz_sled_vec.erase(xyz_sled_vec.begin() + maxradind);
	}
