...
int main(int argc, char** argv) {
	...
	std::ofstream output;
	output.open("/home/tes77/ros_ws/src/roadprintz_camera_calibration/intermediate/random_knockout.csv");
	...
	for(int i = 0; i < s; i++){
		...
		int erase_ind = rand() % xy_pixels_vec.size();
		xy_pixels_vec.erase(xy_pixels_vec.begin() + erase_ind);
		xy_targets_vec.erase(xy_targets_vec.begin() + erase_ind);
		xyz_sled_vec.erase(xyz_sled_vec.begin() + erase_ind);
	}
